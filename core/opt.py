...
def deconv2d(input_, output_dim, kernel=5, stride=2, stddev=0.02, name="deconv2d"):
    with tf.variable_scope(name):
        w = tf.get_variable('w', [kernel, kernel, output_dim, input_.get_shape()[-1]],
              initializer=tf.random_normal_initializer(stddev=stddev))

        output_shape = [input_.get_shape()[0], input_.get_shape()[1]*stride, input_.get_shape()[2]*stride, output_dim]
